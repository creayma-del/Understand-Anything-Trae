...
const SCRIPT = resolve(
  __dirname,
  '../../../understand-anything-trae-plugin/skills/understand/scan-project.mjs',
);
 ...
 * do NOT exclude `.understand-anything-trae/` (the dir is reserved for
...
describe('scan-project.mjs — language detection', () => {
  ...
  it('maps configuration formats (YAML, JSON, JSONC, TOML, Markdown) to their language ids', () => {
    projectRoot = setupTree({
      'a.yaml': 'x: 1\n',
      'b.yml': 'x: 1\n',
      'c.json': '{}\n',
  ...
  it('maps shell + Dockerfile (no extension) to their language ids', () => {
    projectRoot = setupTree({
      'a.sh': 'echo 1\n',
...
describe('scan-project.mjs — category assignment (project-scanner.md Step 4)', () => {
  ...
  it('assigns infra to Dockerfile, .gitlab-ci.yml, .tf, .github/workflows/, Makefile, Jenkinsfile, k8s paths', () => {
    projectRoot = setupTree({
      Dockerfile: 'FROM node:22\n',
  ...
  it('assigns data to SQL, GraphQL, Proto, Prisma', () => {
    projectRoot = setupTree({
  ...
  it('assigns script to shell files (.sh, .bash, .ps1)', () => {
    projectRoot = setupTree({
  ...
  // configs were falling through to `code` / `unknown`. Caught by code review
  // review on PR #204.
