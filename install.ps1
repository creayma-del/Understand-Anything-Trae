<#
.SYNOPSIS
  Understand-Anything-Trae installer for Windows (PowerShell).
...
.EXAMPLE
  ...
  ./install.ps1 trae                 # install for trae
  ./install.ps1 -Update               # pull latest changes
  ./install.ps1 -Uninstall trae      # remove links for trae
#>
...
$RepoUrl    = if ($env:UA_REPO_URL) { $env:UA_REPO_URL } else { 'https://github.com/creayma-del/Understand-Anything-Trae.git' }
$RepoDir    = if ($env:UA_DIR)      { $env:UA_DIR }      else { Join-Path $HOME '.understand-anything-trae\repo' }
$PluginLink = Join-Path $HOME '.understand-anything-trae-plugin'

# Platform table — Target = skills directory; Style = "per-skill" | "folder"
$Platforms = [ordered]@{
    trae    = @{ Target = (Join-Path $HOME '.trae\skills');    Style = 'per-skill' }
    trae-cn = @{ Target = (Join-Path $HOME '.trae-cn\skills'); Style = 'per-skill' }
}

function Show-Usage {
    @"
Understand-Anything-Trae installer (Windows)
...
Environment:
  ...
  UA_DIR        Override clone destination (default: %USERPROFILE%\.understand-anything-trae\repo)
"@
}
...
function Get-SkillsRoot { Join-Path $RepoDir 'understand-anything-trae-plugin\skills' }
...
function Link-Skills([string]$Target, [string]$Style) {
    ...
    switch ($Style) {
        ...
        'folder' {
            $link = Join-Path $Target 'understand-anything-trae'
...
function Unlink-Skills([string]$Target, [string]$Style) {
    ...
    switch ($Style) {
            ...
            if (Test-Path $skillsRoot) {
                ...
                Get-ChildItem -LiteralPath $Target -Force | ForEach-Object {
                    if ($_.LinkType -eq 'Junction' -or $_.LinkType -eq 'SymbolicLink') {
                        if ($_.Target -match 'understand-anything-trae-plugin[\\/]+skills[\\/]+') {
        ...
        'folder' {
            Remove-Reparse (Join-Path $Target 'understand-anything-trae') | Out-Null
        }
    }
}
...
function Link-Plugin-Root {
    if (Test-Path $PluginLink) {
        ...
        $src = Join-Path $RepoDir 'understand-anything-trae-plugin'
...
function Cmd-Install([string]$Id) {
    ...
    Write-Host "`n✓ Installed Understand-Anything-Trae for $Id"
