...
2. Check that `$PROJECT_ROOT/.understand-anything-trae/knowledge-graph.json` exists.
   - If not: report "No existing knowledge graph found. Run `/understand` first to create one." and **STOP**.

3. Check that `$PROJECT_ROOT/.understand-anything-trae/meta.json` exists and read `gitCommitHash`.
...
9. **Apply `.understandignore` exclusions** (same semantics as `/understand` Step 2.5 in `agents/project-scanner.md`).
   ...
   1. If neither `$PROJECT_ROOT/.understand-anything-trae/.understandignore` nor `$PROJECT_ROOT/.understandignore` exists, the step 7 extension filter is sufficient — skip to Phase 1.

   2. Write the step 7 file list to `$PROJECT_ROOT/.understand-anything-trae/intermediate/changed-files-pre.json` as a JSON array of relative paths.

   3. Resolve `$PLUGIN_ROOT`:
      - Use `$PLUGIN_ROOT` if set (Trae's hook context sets this).
      - Otherwise try `$HOME/.understand-anything-trae-plugin`.
      - Validate the chosen candidate by checking `$candidate/packages/core/dist/ignore-filter.js` exists.
      - If neither resolves: report "Cannot locate plugin install at `$PLUGIN_ROOT` or `$HOME/.understand-anything-trae-plugin`; auto-update aborted. Run `/understand` to re-baseline." and **STOP**. Do **not** silently skip — silent skip reproduces issue #153.

   4. Write `$PROJECT_ROOT/.understand-anything-trae/intermediate/ignore-filter.mjs`:
      ```javascript
      ...
      writeFileSync(
        path.join(PROJECT_ROOT, '.understand-anything-trae/intermediate/changed-files.json'),
   ...
   5. Run it:
      ```bash
      node $PROJECT_ROOT/.understand-anything-trae/intermediate/ignore-filter.mjs \
        "$PLUGIN_ROOT" \
        $PROJECT_ROOT/.understand-anything-trae/intermediate/changed-files-pre.json
      ```

   6. Read `$PROJECT_ROOT/.understand-anything-trae/intermediate/changed-files.json`. Pass the `kept` array as the input file list for Phase 1's fingerprint-check script.
...
1. Write and execute a Node.js script (`$PROJECT_ROOT/.understand-anything-trae/intermediate/fingerprint-check.mjs`):

```javascript
// The script should:
// 1. Read fingerprints.json from .understand-anything-trae/fingerprints.json
...
//  6. Write result to .understand-anything-trae/intermediate/change-analysis.json
```
...
2. Read `.understand-anything-trae/intermediate/change-analysis.json`.
...
1. Read the existing knowledge graph from `$PROJECT_ROOT/.understand-anything-trae/knowledge-graph.json`.
...
3. For each batch, dispatch a subagent using the `file-analyzer` agent definition (at `agents/file-analyzer.md`). Append:
   ...
   > Languages: `<languages>`
   > Batch index: `1`
   > Write output to: `$PROJECT_ROOT/.understand-anything-trae/intermediate/batch-1.json`
   >
...
2. Write updated metadata to `$PROJECT_ROOT/.understand-anything-trae/meta.json`:
   ```json
   {
...
3. **Update fingerprints (LOAD-PATCH-SAVE, not OVERWRITE).**
   ...
   const fpPath = path.join(PROJECT_ROOT, '.understand-anything-trae', 'fingerprints.json');
...
4. Clean up intermediate files:
   ```bash
   rm -rf $PROJECT_ROOT/.understand-anything-trae/intermediate
   ```
...
5. Report a summary:
   ...
   - Path to output: `$PROJECT_ROOT/.understand-anything-trae/knowledge-graph.json`
