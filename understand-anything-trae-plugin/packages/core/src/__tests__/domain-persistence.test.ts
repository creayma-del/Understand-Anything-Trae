...
describe("domain graph persistence", () => {
  ...
  it("saves to domain-graph.json, not knowledge-graph.json", () => {
    saveDomainGraph(testRoot, domainGraph);
    const domainPath = join(testRoot, ".understand-anything-trae", "domain-graph.json");
    const structuralPath = join(testRoot, ".understand-anything-trae", "knowledge-graph.json");
