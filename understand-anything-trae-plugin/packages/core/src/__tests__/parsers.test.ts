...
describe("YAMLConfigParser", () => {
  ...
  it("declares yaml as a supported language", () => {
    expect(parser.languages).toEqual(expect.arrayContaining(["yaml"]));
  });
...
describe("JSONConfigParser", () => {
  ...
  it("declares json and jsonc as supported languages", () => {
    expect(parser.languages).toEqual(["json", "jsonc"]);
  });
