...
node $PLUGIN_ROOT/skills/understand/scan-project.mjs \
  "$PROJECT_ROOT" \
  "$PROJECT_ROOT/.understand-anything-trae/tmp/ua-scan-files.json"
```
...
| `Dockerfile`, `Dockerfile.*`, `Makefile`, `Procfile`, `Vagrantfile`, `.gitlab-ci.yml`, `.dockerignore`, `.github/workflows/*`, `.circleci/*`, `*.k8s.yml`/`*.k8s.yaml` | `infra` |
| `.md`, `.mdx`, `.rst`, `.txt`, `.text` (except `LICENSE`) | `docs` |
| `.yaml`, `.yml`, `.json`, `.jsonc`, `.toml`, `.cfg`, `.ini`, `.env`, `.properties`, `.csproj`, `.sln`, `.mod`, `.sum` | `config` |
| `.tf`, `.tfvars` | `infra` |
...
**Priority rule:** most-specific wins. Filename / path rules fire before extension rules — e.g., `.github/workflows/ci.yml` is `infra` (not `config`); `LICENSE` is `code` (not `docs`).

**`.understandignore` behavior:** the bundled script reads `.understandignore` and `.understand-anything-trae/.understandignore` if present and merges them with the hardcoded defaults via `createIgnoreFilter`. `!`-negation overrides defaults (`!dist/` would re-include `dist/` files). The `filteredByIgnore` counter measures only user-driven drops, not baseline default drops.
...
```bash
mkdir -p $PROJECT_ROOT/.understand-anything-trae/tmp
cat > $PROJECT_ROOT/.understand-anything-trae/tmp/ua-import-map-input.json << 'ENDJSON'
{
...
node $PLUGIN_ROOT/skills/understand/extract-import-map.mjs \
  $PROJECT_ROOT/.understand-anything-trae/tmp/ua-import-map-input.json \
  $PROJECT_ROOT/.understand-anything-trae/tmp/ua-import-map-output.json
```
...
1. `$PROJECT_ROOT/.understand-anything-trae/tmp/ua-scan-files.json` — output of `scan-project.mjs` (file list with language, sizeLines, fileCategory; plus `totalFiles`, `filteredByIgnore`, `estimatedComplexity`).
2. `$PROJECT_ROOT/.understand-anything-trae/tmp/ua-import-map-output.json` — output of `extract-import-map.mjs` (the `importMap` field).
...
1. Create the output directory: `mkdir -p <project-root>/.understand-anything-trae/intermediate`
2. Write the JSON to: `<project-root>/.understand-anything-trae/intermediate/scan-result.json`
