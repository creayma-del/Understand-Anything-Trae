...
- Files named `Cargo.toml`, `go.mod`, `Gemfile`, `composer.json` -> `config` (language-level project config)
...
- Detect multi-environment configurations (e.g., Dockerfile.dev, Dockerfile.prod)
...
"deploymentTopology": {
    "hasDockerfile": true,
    "hasCI": true,
    "infraFiles": ["Dockerfile", ".github/workflows/ci.yml"]
  }
```
...
```bash
cat > $PROJECT_ROOT/.understand-anything-trae/tmp/ua-arch-input.json << 'ENDJSON'
{
...
```bash
node $PROJECT_ROOT/.understand-anything-trae/tmp/ua-arch-analyze.js $PROJECT_ROOT/.understand-anything-trae/tmp/ua-arch-input.json $PROJECT_ROOT/.understand-anything-trae/tmp/ua-arch-results.json
```
...
After the script completes, read `$PROJECT_ROOT/.understand-anything-trae/tmp/ua-arch-results.json`. Use the structural analysis as the primary input for your layer decisions. Do NOT re-read source files or re-analyze imports -- trust the script's results entirely.
...
- **Infrastructure layer:** Create if the project has Dockerfiles or other deployment files. Include all `service` and `resource` type nodes.
- **CI/CD layer:** Create if the project has CI/CD configs (.github/workflows, .gitlab-ci.yml). Include all `pipeline` type nodes. May be merged with Infrastructure if few files.
...
- **Data layer:** Create if the project has SQL, GraphQL, or other schema files. Include `table`, `schema`, and `endpoint` type nodes. May be merged with an existing "Data" or "Models" layer.
...
|---|---|
| Dockerfile | `layer:infrastructure` |
| .github/workflows/*, .gitlab-ci.yml | `layer:ci-cd` or merge into `layer:infrastructure` |
...
| *.graphql | `layer:data` or `layer:types` |
...
[
  ...
  },
  {
    ...
    "nodeIds": ["service:Dockerfile", "pipeline:.github/workflows/ci.yml"]
  },
  {
...
1. Write the JSON array to: `<project-root>/.understand-anything-trae/intermediate/layers.json`
