...
| `definitions` | `.env`, GraphQL | `schema:` for graphql; skip for env | `kind` field tells you what each definition is |
| `services` | Dockerfile | `service:<path>:<name>` | One node per stage |
...
| `infra` | `service` | For Dockerfiles |
| `infra` | `pipeline` | For CI/CD configs (.github/workflows, .gitlab-ci) |
| `infra` | `resource` | For CloudFormation, Vagrant |
| `data` | `table` | For SQL files defining tables |
| `data` | `schema` | For GraphQL schema definitions |
...
- `service`: Dockerfile
- `pipeline`: .github/workflows/*, .gitlab-ci.yml, .circleci/*
- `resource`: CloudFormation templates, Vagrantfile
...
- **GraphQL schemas:** Create `defines_schema` edges to the code files that implement the resolvers or service handlers.
...
**Step B — Decide split.**
- If `nodeCount ≤ 60` AND `edgeCount ≤ 120`: write ONE file to `.understand-anything-trae/intermediate/batch-<batchIndex>.json`. Done. Skip to Step F.
...
Write part `k` (1-indexed) to `.understand-anything-trae/intermediate/batch-<batchIndex>-part-<k>.json`. Each part is a valid GraphFragment: `{ "nodes": [...], "edges": [...] }`.
