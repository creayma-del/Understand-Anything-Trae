...
Analyze the current codebase and produce a `knowledge-graph.json` file in `.understand-anything-trae/`. This file powers the interactive dashboard for exploring the project's architecture.

## Options

- `$ARGUMENTS` may contain:
  - `--full` — Force a full rebuild, ignoring any existing graph
  - `--auto-update` — Enable automatic graph updates on commit (writes `autoUpdate: true` to `.understand-anything-trae/config.json`)
  - `--no-auto-update` — Disable automatic graph updates (writes `autoUpdate: false` to `.understand-anything-trae/config.json`)
  - `--review` — Run full LLM graph-reviewer instead of inline deterministic validation
  - `--language <lang>` — Generate all textual content (summaries, descriptions, tags, titles, languageNotes, languageLesson) in the specified language. Accepts `zh` or `en`. Friendly name `chinese` maps to `zh`, `english` maps to `en`. Defaults to `zh` (Chinese). Stores preference in `.understand-anything-trae/config.json` for consistency across incremental updates.
...
1. **Resolve `PROJECT_ROOT`:**
   ...
   - **Worktree redirect.** If `PROJECT_ROOT` is inside a git worktree (not the main checkout), redirect output to the main repository root. Worktrees managed by Trae are ephemeral — `.understand-anything-trae/` written there is destroyed when the session ends, taking the knowledge graph with it (issue #133). Detect a worktree by comparing `git rev-parse --git-dir` against `git rev-parse --git-common-dir`; in a normal checkout or submodule they resolve to the same path, in a worktree they differ and the parent of `--git-common-dir` is the main repo root.
...
1.5. **Ensure the plugin is built.** Later phases invoke Node scripts that import `@understand-anything-trae/core`. On a fresh install `packages/core/dist/` does not exist yet — build once.

   **Important:** do **not** assume the plugin root is simply two directories above the skill path string. In many installations `~/.trae/skills/understand` is a symlink into the real plugin checkout. Fall back to universal symlinks and skill symlink resolution.

   Resolve the plugin root like this:

   ```bash
   TRAE_SKILL_REAL=$(realpath ~/.trae/skills/understand 2>/dev/null || readlink -f ~/.trae/skills/understand 2>/dev/null || echo "")
   TRAE_SELF_RELATIVE=$([ -n "$TRAE_SKILL_REAL" ] && cd "$TRAE_SKILL_REAL/../.." 2>/dev/null && pwd || echo "")
   TRAE_CN_SKILL_REAL=$(realpath ~/.trae-cn/skills/understand 2>/dev/null || readlink -f ~/.trae-cn/skills/understand 2>/dev/null || echo "")
   ...
   if [ -z "$PLUGIN_ROOT" ]; then
     ...
     echo "  - ${TRAE_SELF_RELATIVE:-<unresolved path derived from ~/.trae/skills/understand>}"
     echo "  - ${TRAE_CN_SELF_RELATIVE:-<unresolved path derived from ~/.trae-cn/skills/understand>}"
     echo "Make sure the plugin is installed correctly."
     exit 1
   fi

   if [ ! -f "$PLUGIN_ROOT/packages/core/dist/index.js" ]; then
     cd "$PLUGIN_ROOT" && (pnpm install --frozen-lockfile 2>/dev/null || pnpm install) && pnpm --filter @understand-anything-trae/core build
   fi
   ```
...
3. Create the intermediate and temp output directories:
   ```bash
   mkdir -p $PROJECT_ROOT/.understand-anything-trae/intermediate
   mkdir -p $PROJECT_ROOT/.understand-anything-trae/tmp
   ```
3.5. **Auto-update configuration:**
    - If `--auto-update` is in `$ARGUMENTS`: write `{"autoUpdate": true}` to `$PROJECT_ROOT/.understand-anything-trae/config.json`
    - If `--no-auto-update` is in `$ARGUMENTS`: write `{"autoUpdate": false}` to `$PROJECT_ROOT/.understand-anything-trae/config.json`
 ...
 3.6. **Language configuration:**
    ...
    - If `--language` is NOT specified:
      - Check `$PROJECT_ROOT/.understand-anything-trae/config.json` for an existing `outputLanguage` field. If present, use that.
      - If no stored preference, default to `zh` (Chinese).
    - If `--language` IS specified:
      - Update `$PROJECT_ROOT/.understand-anything-trae/config.json` with the new language: merge `{"outputLanguage": "<lang>"}` into existing config.
 ...
 4. **Check for subdomain knowledge graphs to merge:**
   List all `*knowledge-graph*.json` files in `$PROJECT_ROOT/.understand-anything-trae/` **excluding** `knowledge-graph.json` itself (e.g. `frontend-knowledge-graph.json`, `backend-knowledge-graph.json`). If any subdomain graphs exist, run the merge script bundled with this skill (located next to this SKILL.md file — use the skill directory path, not the project root):
   ```bash
...
5. Check if `$PROJECT_ROOT/.understand-anything-trae/knowledge-graph.json` exists. If it does, read it.
6. Check if `$PROJECT_ROOT/.understand-anything-trae/meta.json` exists. If it does, read it to get `gitCommitHash`.
7. **Decision logic:**
   ...
   **Review-only path:** Copy the existing `knowledge-graph.json` to `$PROJECT_ROOT/.understand-anything-trae/intermediate/assembled-graph.json`, then jump directly to Phase 6 step 3.
...
8. **Collect project context for subagent injection:**
   ...
   - Read the primary package manifest (`package.json`, `pyproject.toml`) if it exists. Store as `$MANIFEST_CONTENT`.
   - Capture the top-level directory tree:
     ```bash
     find $PROJECT_ROOT -maxdepth 2 -type f -not -path '*/node_modules/*' -not -path '*/.git/*' -not -path '*/dist/*' | head -100
     ```
     Store as `$DIR_TREE`.
   - Detect the project entry point by checking for common patterns (in order): `src/index.ts`, `src/main.ts`, `src/App.tsx`, `index.js`. Store first match as `$ENTRY_POINT`.
...
1. Check if `$PROJECT_ROOT/.understand-anything-trae/.understandignore` exists.
2. **If it does NOT exist**, generate a starter file:
   - Run the following Node.js one-liner in `$PROJECT_ROOT` (reads `.gitignore` and deduplicates against built-in defaults):
     ...
     const outDir = path.join(root, '.understand-anything-trae');
     if (!fs.existsSync(outDir)) fs.mkdirSync(outDir, { recursive: true });
     fs.writeFileSync(path.join(outDir, '.understandignore'), header + body);
     "
     ```
   - Report to the user:
     > Generated `.understand-anything-trae/.understandignore` with suggested exclusions based on your project structure. Please review it and uncomment any patterns you'd like to exclude from analysis. When ready, confirm to continue.
   - **Wait for user confirmation before proceeding.**
3. **If it already exists**, report:
   > Found `.understand-anything-trae/.understandignore`. Review it if needed, then confirm to continue.
   - **Wait for user confirmation before proceeding.**
...
> Write output to: `$PROJECT_ROOT/.understand-anything-trae/intermediate/scan-result.json`

After the subagent completes, read `$PROJECT_ROOT/.understand-anything-trae/intermediate/scan-result.json` to get:
...
This script reads all `batch-*.json` files (including `batch-<i>-part-<k>.json` produced by file-analyzers that split their output) from `$PROJECT_ROOT/.understand-anything-trae/intermediate/`, then in one pass:
...
```bash
git diff <lastCommitHash>..HEAD --name-only > $PROJECT_ROOT/.understand-anything-trae/tmp/changed-files.txt
```

Run compute-batches with `--changed-files`:
```bash
node <SKILL_DIR>/compute-batches.mjs $PROJECT_ROOT \
  --changed-files=$PROJECT_ROOT/.understand-anything-trae/tmp/changed-files.txt
```
...
> Project root: `$PROJECT_ROOT`
> Write output to: `$PROJECT_ROOT/.understand-anything-trae/intermediate/layers.json`
> Project: `<projectName>` — `<projectDescription>`
>
...
> Project root: `$PROJECT_ROOT`
> Write output to: `$PROJECT_ROOT/.understand-anything-trae/intermediate/tour.json`
> Project: `<projectName>` — `<projectDescription>`
> Languages: `<languages>`
>
...
2. Write the assembled graph to `$PROJECT_ROOT/.understand-anything-trae/intermediate/assembled-graph.json`.
...
Write the following Node.js script to `$PROJECT_ROOT/.understand-anything-trae/tmp/ua-inline-validate.cjs`:
...
Execute it:
```bash
node $PROJECT_ROOT/.understand-anything-trae/tmp/ua-inline-validate.cjs \
  "$PROJECT_ROOT/.understand-anything-trae/intermediate/assembled-graph.json" \
  "$PROJECT_ROOT/.understand-anything-trae/intermediate/review.json"
```
...
> Validate the knowledge graph at `$PROJECT_ROOT/.understand-anything-trae/intermediate/assembled-graph.json`.
> Project root: `$PROJECT_ROOT`
> Read the file and validate it for completeness and correctness.
> Write output to: `$PROJECT_ROOT/.understand-anything-trae/intermediate/review.json`

---

4. Read `$PROJECT_ROOT/.understand-anything-trae/intermediate/review.json`.
...
1. Write the final knowledge graph to `$PROJECT_ROOT/.understand-anything-trae/knowledge-graph.json`.
...
2. **Generate structural fingerprints baseline.** This creates the basis for future automatic incremental updates and **must succeed before `meta.json` is written** — otherwise auto-update sees a fresh commit hash with no fingerprints to compare against, classifies every file as STRUCTURAL, and escalates to `FULL_UPDATE` on every subsequent commit (issue #152).
   ...
   ```bash
   cat > $PROJECT_ROOT/.understand-anything-trae/intermediate/fingerprint-input.json <<EOF
   {
   ...
   node <SKILL_DIR>/build-fingerprints.mjs \
     $PROJECT_ROOT/.understand-anything-trae/intermediate/fingerprint-input.json
   ```
...
3. Write metadata to `$PROJECT_ROOT/.understand-anything-trae/meta.json` (only after step 2 succeeded):
   ```json
   {
...
4. Clean up intermediate files, **preserving `scan-result.json`** so future incremental runs can skip Phase 1 SCAN (see issue #293):
   ...
   INTER="$PROJECT_ROOT/.understand-anything-trae/intermediate"
   if [ -d "$INTER" ]; then
     find "$INTER" -mindepth 1 -maxdepth 1 -not -name 'scan-result.json' -exec rm -rf {} +
   fi
   rm -rf $PROJECT_ROOT/.understand-anything-trae/tmp
   ```
...
5. Report a summary to the user containing:
   ...
   - Path to the output file: `$PROJECT_ROOT/.understand-anything-trae/knowledge-graph.json`
