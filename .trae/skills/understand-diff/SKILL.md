...
Analyze the current code changes against the knowledge graph at `.understand-anything-trae/knowledge-graph.json`.
...
1. Check that `.understand-anything-trae/knowledge-graph.json` exists. If not, tell the user to run `/understand` first.
...
8. **Write diff overlay for dashboard** — after producing the analysis, write the diff data to `.understand-anything-trae/diff-overlay.json` so the dashboard can visualize changed and affected components. The file contains:
   ```json
   {
     "version": "1.0.0",
