...
- If a knowledge graph already exists (`.understand-anything-trae/knowledge-graph.json`), derives domain knowledge from it (cheap, no file scanning)
...
**Important:** do **not** assume the plugin root is simply two directories above the skill path string. In many installations `~/.trae/skills/understand-domain` is a symlink into the real plugin checkout. Fall back to universal symlinks and skill symlink resolution.

Resolve the plugin root like this:

```bash
TRAE_SKILL_REAL=$(realpath ~/.trae/skills/understand-domain 2>/dev/null || readlink -f ~/.trae/skills/understand-domain 2>/dev/null || echo "")
TRAE_SELF_RELATIVE=$([ -n "$TRAE_SKILL_REAL" ] && cd "$TRAE_SKILL_REAL/../.." 2>/dev/null && pwd || echo "")
TRAE_CN_SKILL_REAL=$(realpath ~/.trae-cn/skills/understand-domain 2>/dev/null || readlink -f ~/.trae-cn/skills/understand-domain 2>/dev/null || echo "")
...
if [ -z "$PLUGIN_ROOT" ]; then
  ...
  echo "  - ${TRAE_SELF_RELATIVE:-<unresolved path derived from ~/.trae/skills/understand-domain>}"
  echo "  - ${TRAE_CN_SELF_RELATIVE:-<unresolved path derived from ~/.trae-cn/skills/understand-domain>}"
...
1. Check if `$PROJECT_ROOT/.understand-anything-trae/knowledge-graph.json` exists
...
1. Run the preprocessing script bundled with this skill, passing `$PROJECT_ROOT` from Phase 0:
   ...
   ```
   This outputs `$PROJECT_ROOT/.understand-anything-trae/intermediate/domain-context.json` containing:
...
1. Read `$PROJECT_ROOT/.understand-anything-trae/knowledge-graph.json`
...
3. The agent writes its output to `$PROJECT_ROOT/.understand-anything-trae/intermediate/domain-analysis.json`
...
4. Save to `$PROJECT_ROOT/.understand-anything-trae/domain-graph.json`
5. Clean up `$PROJECT_ROOT/.understand-anything-trae/intermediate/domain-analysis.json` and `$PROJECT_ROOT/.understand-anything-trae/intermediate/domain-context.json`
