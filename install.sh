#!/usr/bin/env bash
# Understand-Anything-Trae installer (macOS / Linux)
#
# Usage:
...
#
# Curl-pipe usage:
#   curl -fsSL https://raw.githubusercontent.com/creayma-del/Understand-Anything-Trae/main/install.sh | bash
...
#   UA_DIR       Override clone destination (default: $HOME/.understand-anything-trae/repo)

set -euo pipefail

REPO_URL="${UA_REPO_URL:-https://github.com/creayma-del/Understand-Anything-Trae.git}"
REPO_DIR="${UA_DIR:-$HOME/.understand-anything-trae/repo}"
PLUGIN_LINK="$HOME/.understand-anything-trae-plugin"
...
skills_root() { printf '%s\n' "$REPO_DIR/understand-anything-trae-plugin/skills"; }
...
link_skills() {
  ...
  case "$style" in
    per-skill)
      ...
      ;;
    folder)
      ln -sfn "$root" "$target/understand-anything-trae"
      printf '  ✓ %s → %s\n' "$target/understand-anything-trae" "$root"
      ;;
    *)
    ...
    per-skill)
      if [[ -d "$(skills_root)" ]]; then
        ...
        for link in "$target"/*; do
          ...
          [[ "$resolved" == *"/understand-anything-trae-plugin/skills/"* ]] || continue
          rm -f "$link"
        done
      fi
      ;;
    folder)
      [[ -L "$target/understand-anything-trae" ]] && rm -f "$target/understand-anything-trae"
      ;;
  esac
}
...
link_plugin_root() {
  if [[ -L "$PLUGIN_LINK" || -e "$PLUGIN_LINK" ]]; then
    ...
    ln -s "$REPO_DIR/understand-anything-trae-plugin" "$PLUGIN_LINK"
    printf '  ✓ %s → %s\n' "$PLUGIN_LINK" "$REPO_DIR/understand-anything-trae-plugin"
  fi
}
...
cmd_install() {
  ...
  printf '\n✓ Installed Understand-Anything-Trae for %s\n' "$id"
...
usage() {
  cat <<USAGE
Understand-Anything-Trae installer
